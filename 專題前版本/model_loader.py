...
class ModelManager:
    ...
    def _load_tts_cosvoice(self):
        ...
        try:
            ...
            # 🚀 [新增] 使用 torch.compile 加速推理 (這行是關鍵)
            # 注意：第一次執行時會花約 1 分鐘進行編譯，之後就會變快
            if sys.platform != "win32":  # Linux/Mac 效果最好
                try:
                    print("   ⚡ 正在編譯 CosyVoice 模型以加速 (需時約1分鐘)...")
                    model.model = torch.compile(model.model)
                    print("   ✅ 編譯完成")
                except Exception as e:
                    print(f"   ⚠️ 編譯失敗 (不影響使用): {e}")
            else:
                # Windows 下 torch.compile 支援度較差，但可以嘗試
                print("   ℹ️  Windows 系統跳過 torch.compile (避免相容性問題)")
    ...
    def _load_emotion_models(self):
        ...
        try:
            ...
            emotion_text_zh = pipeline(
                "text-classification",
                model=config.EMOTION_TEXT_ZH_MODEL,
                top_k=None,
                device=-1
            )
            print("   ✅ 中文文字情緒模型載入成功")
            ...
            emotion_text_en = pipeline(
                "text-classification",
                model=config.EMOTION_TEXT_EN_MODEL,
                top_k=None,
                device=-1
            )
            print("   ✅ 英文文字情緒模型載入成功")
            ...
            emotion_audio_model = AutoModel(
                ...
                disable_update=True  # 這樣可以避免每次啟動都去檢查遠端版本，加快啟動速度
            )
            print("   ✅ 語音情緒辨識模型載入成功\n✅ 情緒辨識模型已就緒")
