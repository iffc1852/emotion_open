...
class AIServices:
    ...
    def select_personality_auto(self, text_emotion: str, audio_emotion: str, facial_emotion: str,
            ...
            # 這裡先不檢查趨勢，把趨勢判斷留給 server.py 決定人格後，或者在這裡只做人格映射
            # 為了避免邏輯打架，我們這裡只負責算分，回傳贏家
            ...
            if not scores:
                ...
                print(f"   {msg}")
                logs.append(msg)
                return config.EMOTION_TO_PERSONALITY.get(emotion_memory.get_dominant_emotion(), "humorous"), logs

            # 找出最高分
            primary = max(scores.items(), key=lambda x: x[1])[0]
            result_msg = f"🏆 最高分: {primary} (總分: {scores[primary]:.4f})"
            print(f"   {result_msg}")
            logs.append(result_msg)

            # 記憶趨勢判斷
            emotion_trend = emotion_memory.get_emotion_trend()
            if emotion_trend in ["worsening", "persistent_worsening"]:
                trend_msg = "🚨 趨勢惡化，強制切換【安撫型】"
                print(f"   {trend_msg}")
                logs.append(trend_msg)
                return "comforting", logs
