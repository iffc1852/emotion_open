# server.py (本地端電腦的 Web API) - 修正情緒記憶更新
    ...
    try:
        ...
        if is_command:
            if target == 'reset':
                ...
                new_manual_personality = None
                reply = "好的，我已將人格模式切換回自動偵測，隨時聽候您的指示。"
            else:
        ...
        state['memory'].add_emotion(text_emo, audio_emo, facial_emo)

        # 人格選擇
        if not state['manual_personality']:
            selected_personality = services.select_personality_auto(
                text_emo, audio_emo, facial_emo, state['memory'],
...
def download_tts_audio():
    ...
    if not filename: return jsonify({"error": "Missing filename parameter"}), 400
