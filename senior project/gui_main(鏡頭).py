...
class CameraThread(QThread):
    ...
    def run(self):
        # 1. 抓取預設的第一台視訊鏡頭 (把 self.cap 改回 cap，統一變數)
        cap = cv2.VideoCapture(0)

        #  [效能救星] 強制將鏡頭解析度壓制在 480p，避免 MediaPipe 榨乾 CPU 拖累 TTS！
        cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
        cap.set(cv2.CAP_PROP_FPS, 10)

        if not cap.isOpened():
            print("⚠️ 找不到視訊鏡頭！請確認是否沒接好或被其他軟體佔用。")
            return
        ...
        while self.running:
            ...
            # ==========================================
            # 🚀 鏡頭專用防當機邏輯：
            # 鏡頭抓不到畫面時，不可以使用倒帶，而是休息一下再試，避免燒壞 CPU
            if not ret:
                print("⚠️ 鏡頭畫面讀取失敗...")
                time.sleep(0.1)  # 休息 0.1 秒避免無限死迴圈
                continue
                # ==========================================
        ...
        while self.running:
            try:
                ...
                if is_command:
                    ...
                    # 改用最新的 CosyVoice 記憶體直出生成，並用 sounddevice 播放
                    audio_data, _ = self.assistant.services.synthesize_cosvoice_audio(reply, target_p)
