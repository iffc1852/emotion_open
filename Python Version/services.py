...
class AIServices:
    ...
    def text_to_speech_cosvoice(self, text: str, output_path: str, personality: str) -> bool:
        """(TTS) CosyVoice 2.0 - 只傳入純文本，完全依靠 LLM 輸出的情緒線索"""
        try:
            from cosyvoice.utils.file_utils import load_wav
            ...
            # 2. 準備參考文本 (Zero-shot 必需的逐字稿)
            # 這裡只讀取 config 裡的 COSVOICE_REFERENCE_TEXT，不再嘗試讀取 personality config
            ...
            # 4. 【關鍵】格式化 CosyVoice 輸入字串 (只傳入純文本，不含任何標籤)
            if target_lang == "zh":
                # 轉簡體是為了提高模型發音準確性
            ...
            # 5. 執行推論 (此時情緒完全來自 LLM 在 'text' 裡寫的 '哈哈'、'...' 等)
